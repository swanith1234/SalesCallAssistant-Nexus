...
import { fetchLatestAnalysis, fetchMessages } from "@/lib/analysis-service";
...
export default function LiveCall() {
  ...
  const connectToRoom = async () => {
    ...
    try {
      ...
      roomObj.on(RoomEvent.Reconnecting, () => setStatus("Reconnecting..."));
      roomObj.on(RoomEvent.Reconnected, () => setStatus("Connected"));
  ...
  const disconnect = async () => {
    ...
    if (roomName) {
      try {
        await fetch(`http://localhost:8000/save-session?room_id=${roomName}`, {
          method: "POST",
        });
